...
# ================== CONFIG ==================
HIVE_CATALOG = "minio"
BRONZE_SCHEMA = "temp"  # Ensure this is the correct schema you want for your bronze tables.
...
ICEBERG_PREFIX = "bronze/iceberg"  # Changed to point to the bronze folder

ICEBERG_CATALOG = "iceberg"
ICEBERG_SCHEMA = "bronze"  # Changed to bronze schema
...
def register_single_table(schema_name: str, table_name: str, trino_conn_id: str = "trino_default"):
    ...
    iceberg_table_fqn = f'{ICEBERG_CATALOG}.{ICEBERG_SCHEMA}.bronze_{schema_name.lower()}_{table_name.lower()}'
...
# ================== DAG ==================
with DAG(
    dag_id="register_bronze_single_table",
    start_date=days_ago(1),
    schedule=None,
    catchup=False,
    tags=["bronze", "register", "ctas"],
) as dag:
    PythonOperator(
