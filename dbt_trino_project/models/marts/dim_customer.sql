...
email_raw as (
    ...
    from {{ source('silver_src', 'silver_person_emailaddress') }}
),

phone_raw as (
    select
        businessentityid, phonenumber, phonenumbertypeid, modifieddate
    from {{ source('silver_src', 'silver_person_personphone') }}
),

bea_raw as (
    select
        businessentityid, addressid, addresstypeid, rowguid, modifieddate
    from {{ source('silver_src', 'silver_person_businessentityaddress') }}
),
...
address_raw as (
    ...
    from {{ source('silver_src', 'silver_person_address') }}
),
...
stateprovince_raw as (
    ...
    from {{ source('silver_src', 'silver_person_stateprovince') }}
),

countryregion_raw as (
    select
        countryregioncode, name as countryregion_name, modifieddate
    from {{ source('silver_src', 'silver_person_countryregion') }}
),

{# ======================================================
  2. CLEAN TÊN / EMAIL / PHONE
  ====================================================== #}
...
{# ======================================================
  4. FACT DIM_CUSTOMER "CLEAN" (TẠO FULL NAME VÀ SỬA LỖI BLANK)
  ====================================================== #}
